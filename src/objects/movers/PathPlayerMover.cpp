...
void PathPlayerMover::move(float deltaTime) {
    while (deltaTime > 0.0f) {
        ...
        if (std::holds_alternative<Player::Moving>(state)) {
            player.setState(moveMovingPlayer(deltaTime, std::get<Player::Moving>(state)));
        }
        else if (std::holds_alternative<Player::JumpingOnPath>(state)) {
            player.setState(moveJumpingPlayer(deltaTime, std::get<Player::JumpingOnPath>(state)));
        }
...
Player::State PathPlayerMover::moveMovingPlayer(float& deltaTime, Player::Moving& moving) {
...
Player::State PathPlayerMover::moveJumpingPlayer(float& deltaTime, Player::JumpingOnPath& jumping) {
